...
row_func <- function(df){
  ...
  for(i in 1:nrow(df)) df$row[i] <- LETTERS[1:16][str_detect(df$Location[i], LETTERS[1:16])]
  return(df)    
  
}
countsdf <- row_func(countsdf)
...
ggplot(countsdf, aes(Loc, antigen))+ geom_tile(aes(fill=count_grp), col='grey')+
  ...
  scale_x_discrete(breaks=breaks, labels=LETTERS[1:length(unique(countsdf$row))])+
...
meddf$beadcount[meddf$beadcount=='b	%50'] <- ">50"
